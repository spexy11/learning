...
type ModuleView<N extends ModuleNames> =
  Awaited<ReturnType<(typeof registry)['views'][N]>> extends View<infer T> ? View<T> : never

type Props<N extends ModuleNames, K extends keyof ModuleView<N> = keyof ModuleView<N>> = {
...
function loadComponent<N extends ModuleNames>(name: N) {
  return lazy(async () => {
    ...
    return {
      default: (props) =>
        (<Dynamic component={view[props.step]} {...props} />) as Component<Props<N>>,
    }
  })
}
...
const submitExerciseFn = async <N extends ModuleNames>(
  ...
  const attempt = [{ step, state: Object.fromEntries(form.entries()) }, ...(exercise.attempt ?? [])]
...
export default function Exercise<N extends ModuleNames>(
  ...
  createEffect(() => {
    const result = submission.result as Exercise<Module<N>['schema']> | undefined
    if (result) {
  ...
  return (
    <div class="flex flex-col-reverse">
      ...
      <Suspense fallback={<p>Correction en cours...</p>}>
        <For each={attempt}>
          {(part, i) => (
            <Step index={attempt.length - i()} grade={grades()?.[i()]?.score} disabled>
              <Dynamic component={component()} {...props} {...part} feedback={feedback()?.[i()]} />
            </Step>
          )}
        </For>
      </Suspense>
