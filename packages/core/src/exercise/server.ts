...
async function gradeExercise<T extends Schema, F extends Feedback<T>>(
  ...
  const graded = await mapAsync(attempt, async (part, i) => {
    ...
    for await (const chunk of gen) {
      if (Array.isArray(chunk)) score = [score[0] + chunk[0], score[1] + chunk[1]]
      else next = chunk
    }
...
export function createGradeFunction<const R extends ModelRegistry>(registry: R) {
...
export type ServerModule<R extends ModelRegistry, N extends R[number]['schema']['name']> = Extract<
  R[number],
  { schema: { name: N } }
>

export function createFeedbackFunction<const R extends ModelRegistry>(registry: R) {
  ...
  ): Promise<FeedbackReturn<ServerModule<R, N>['schema'], ServerModule<R, N>['feedback'], K>> {
...
const Params = v.record(
  ...
  v.union([
    ...
    v.string(),
    v.pipe(
      v.tuple([v.literal('sample'), v.array(v.union([v.number(), v.string()])), v.number()]),
...
export function createGenerator<const R extends ModelRegistry>(registry: R) {
  return async function (rawExerciseStub: GeneratedExercise<R>): Promise<BaseExercise<R>> {
    let { params, question, ...exercise } = v.parse(GeneratedExercise(registry), rawExerciseStub)
